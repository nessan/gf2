...
public:
    ...
    static BitVec random(usize len, double p = 0.5, u64 seed = 0) {
        BitVec result{len};
        result.fill_from_random(p, seed);
        return result;
    }
    ...
    auto append(Src src) {
        ...
        this->span(old_size, size()).copy_from(src);
        return *this;
    }

    /// Appends all the bits from *any* `Unsigned` `src` onto the end of the bit-vector and returns this for chaining.
    ///
