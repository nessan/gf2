...
private:
    ...
    // The bit-span begins at this bit-offset inside `m_data[0]`.
    u8 m_offset = 0;
...
public:
    ...
    constexpr void set_word(usize i, word_type value) {
        if constexpr (std::is_const_v<Word>) {
            ...
            gf2::replace_bits(m_data[i], m_offset, m_offset + w0_bits, value << m_offset);
    ...
    /// @}
    /// @name Copying Bits into the Span:
    /// @{
    ...
    /// auto s = v.span(0,10);
    /// s.copy_from(BitVec<u8>::alternating(10));
    /// assert_eq(v.to_string(), "1010101010111111");
    /// ```
    template<BitStore Src>
    auto copy_from(Src const& src) {
        return gf2::copy(src, *this);
    }
    ...
    /// ```
    template<usize N>
    auto copy_from(std::bitset<N> const& src) {
        return gf2::copy(src, *this);
    }
    ...
    /// auto v = BitVec<u8>::ones(16);
    /// auto s = v.span(0,10);
    /// s.fill_from([](usize i) { return i % 2 == 0; });
    /// assert_eq(s.to_string(), "1010101010");
    /// assert_eq(v.to_string(), "1010101010111111");
    /// ```
    auto fill_from(std::invocable<usize> auto f) { return gf2::fill(*this, f); }
    ...
    /// auto t = v.span(2,10);
    /// u64 seed = 1234567890;
    /// s.fill_from_random(0.5, seed);
    /// t.fill_from_random(0.5, seed);
    /// assert(u == v);
    /// ```
    auto fill_from_random(double p = 0.5, u64 seed = 0) { return gf2::fill_random(*this, p, seed); }
