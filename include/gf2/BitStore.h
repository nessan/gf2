...
/// | `data() const`     | Returns a read-only pointer to the first word holding bits in the store.                |
/// | `data()`           | Returns a pointer to the first word holding bits in the store.                          |
/// | `offset()`         | Returns the bit location of the store's first bit element in the `*data()` word.        |
///
/// The offset will always be zero for bit-sets and bit-vectors but typically non-zero for bit-spans, which are views
...
/// They are trivially implemented for bit-sets and bit-vectors, but require some work for bit-spans as "words" are
...
concept BitStore = requires(Store store, const Store const_store) {
    ...
    /// The const `data()` method should return a const pointer to the real first word in the underlying store.
    { const_store.data() } -> std::same_as<const typename Store::word_type*>;

    /// The non-const `data()` method should return a pointer to the real first word in the underlying store.
    /// The pointer may be to const or non-const words depending on whether the store itself is const or not.
    { store.data() } -> std::convertible_to<const typename Store::word_type*>;

    /// The `offset()` method should return the number of bits from the least significant bit of `data()[0]` to the
    ...
    /// `words()` on the fly but all concrete bit-store types cache this value for efficiency.
...
/// ```
/// BitVec v{10};
/// fill(v, [](usize i) { return i % 2 == 0; });
/// assert_eq(v.size(), 10);
/// assert_eq(to_string(v), "1010101010");
/// ```
template<BitStore Store>
constexpr auto&
fill(Store& store, std::invocable<usize> auto f) {
    set_all(store, false);
...
span(Store const& store, usize begin, usize end) {
    ...
    return BitSpan<const word_type>(store.data() + data_index, bit_offset, end - begin);
}
...
span(Store& store, usize begin, usize end) {
    ...
    // Return the read-only bit-span over the underlying word data.
    return BitSpan<word_type>(store.data() + data_index, bit_offset, end - begin);
}
...
join(Lhs const& lhs, Rhs const& rhs) {
    ...
    result.span(0, lhs_size).copy_from(lhs);
    result.span(lhs_size, lhs_size + rhs_size).copy_from(rhs);
    return result;
}
