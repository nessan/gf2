...
// clang-format off
// Forward declare some classes of interest:
template<typename Store> class BitStore;
template<typename Store> class BitRef;
// clang-format on
...
public:
    ...
    using store_pointer = std::conditional_t<is_const, const BitStore<Store>*, BitStore<Store>*>;
...
class SetBitsIter {
private:
    const BitStore<Store>* m_store = nullptr;      // The store we're iterating over
...
public:
    ...
    SetBitsIter(const BitStore<Store>* store, std::optional<usize> idx = std::nullopt) : m_store(store), m_index{idx} {
        // Empty body ...
    }
    ...
    SetBitsIter(const SetBitsIter&) = default;
    SetBitsIter& operator=(SetBitsIter&&) = default;
    SetBitsIter& operator=(const SetBitsIter&) = default;

    // Iterators must be comparable (this is how the range-for loops work).
    constexpr bool operator==(const SetBitsIter& other) const {
        return m_store == other.m_store && m_index == other.m_index;
    }
    ...
    // Decrement the iterator, returning the new value (the `--i` operator).
    constexpr SetBitsIter& operator--() {
        if (m_index.has_value()) m_index = m_store->previous_set(*m_index);
        return *this;
    }
...
public:
    ...
    // Our constructor captures a pointer to the store and sets the index to uninitialized.
    UnsetBitsIter(const BitStore<Store>* store, std::optional<usize> idx = std::nullopt) :
        m_store(store), m_index{idx} {
        // Empty body ...
    }
    ...
    UnsetBitsIter(const UnsetBitsIter&) = default;
    UnsetBitsIter& operator=(UnsetBitsIter&&) = default;
    UnsetBitsIter& operator=(const UnsetBitsIter&) = default;

    // Iterators must be comparable (this is how the range-for loops work).
    constexpr bool operator==(const UnsetBitsIter& other) const {
        return m_store == other.m_store && m_index == other.m_index;
    }
    ...
    // Decrement the iterator, returning the new value (the `--i` operator).
    constexpr UnsetBitsIter& operator--() {
        if (m_index.has_value()) m_index = m_store->previous_unset(*m_index);
        return *this;
    }
...
class WordsIter {
private:
    const BitStore<Store>* m_store = nullptr;
    usize                  m_index = 0;

public:
    // The value type is the word type of the store with any const qualification dropped.
    using value_type = typename store_word<Store>::word_type;

    // The difference type is always a `std::ptrdiff_t` (required by the iterator concept).
    using difference_type = std::ptrdiff_t;

    // Our constructor captures a pointer to the store and sets the index to 0
    WordsIter(const BitStore<Store>* store, usize idx = 0) : m_store(store), m_index{idx} {
        // Empty body ...
    }
    ...
    WordsIter(const WordsIter&) = default;
    WordsIter& operator=(WordsIter&&) = default;
    WordsIter& operator=(const WordsIter&) = default;

    // Iterators must be comparable (this is how the range-for loops work).
    constexpr bool operator==(const WordsIter& other) const {
        return m_store == other.m_store && m_index == other.m_index;
    }
