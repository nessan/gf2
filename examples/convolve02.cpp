// Checks on word-by-word convolution vs element-by-element version over a wide range of inputs.
...
main() {
    ...
    for (auto n = 0uz; n < n_trials; ++n) {
        ...
        if (w1 != w2) {
            ...
            std::println("\nMETHOD MISMATCH, METHOD MISMATCH, METHOD MISMATCH, METHOD MISMATCH!");
            std::println("Input bit-vector u: {}", u);
            std::println("Input bit-vector v: {}", v);
            std::println("Naive convolution:  {}", w1);
            std::println("Faster convolution: {}", w2);
            exit(1);
        }
    }
    std::println("\nHurray! All {} trials matched!", n_trials);
}
