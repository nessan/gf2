...
public:
    ...
    /// ```
    explicit constexpr BitSet(Word word) {
    ...
    /// assert_eq(v.to_string(), "0000000000");
    /// ```
    static constexpr BitSet zeros() { return BitSet{}; }
    ...
    /// assert_eq(v.to_string(), "1111111111");
    /// ```
    static constexpr BitSet ones() { return BitSet{MAX<Word>}; }
    ...
    /// ```
    static constexpr BitSet constant(bool value) { return BitSet{value ? MAX<Word> : Word{0}}; }
    ...
    static constexpr BitSet unit(usize i) {
        gf2_assert(i < N, "Unit axis i = {} should be less than the bit-set size n = {}", i, N);
        BitSet result{};
        result.set(i);
        return result;
    }
    ...
    /// assert_eq(v.to_string(), "1010101010");
    /// ```
    static constexpr BitSet alternating() { return BitSet{gf2::ALTERNATING<Word>}; }
    ...
    /// @}
    /// @name Random BitSet Constructors:
    /// @{

    /// Factory method to generate a bit-set of size `N` where the elements are picked at random.
    ///
    /// The default call `BitSet<>::random()` produces a random bit-set with each bit being 1 with probability
    ...
    ///
    /// @note If `p < 0` then the bit-set is all zeros, if `p > 1` then the bit-set is all ones.
    ///
    /// # Example
    /// ```
    /// u64 seed = 1234567890;
    ...
    /// Factory method to generate a bit-set of size `N` where the elements are from independent fair
    ...
    /// assert(u == v);
    /// ```
    static BitSet seeded_random(u64 seed) { return random(0.5, seed); }
    ...
    /// ```
    static BitSet biased_random(double p) { return random(p, 0); }
    ...
    /// @}
    /// @name Copying Bits into the Store:
    /// @{
    ...
    /// `word_type`. You can use it to convert between different `word_type` stores (e.g., from `BitSet<u32>` to
    /// `BitSet<u8>`) as long as the sizes match.
    ///
    /// # Example
    /// ```
    /// BitSet<10, u64> v;
    ...
    /// synthesise "words" on the fly from adjacent pairs of bit-set words. Nevertheless, almost all the methods
    ...
    ///
    /// On return, `left` contains the bits from the start of the bit-set up to but not including `at` and `right`
    /// contains the bits from `at` to the end of the bit-set. This bit-set itself is not modified.
    ///
    /// This lets one reuse the `left` and `right` destinations without having to allocate new bit-vectors.
    /// This is useful when implementing iterative algorithms that need to split a bit-set into two parts repeatedly.
    ///
    /// @note This method panics if the split point is beyond the end of the bit-set.
    ///
    /// # Example
    /// ```
    /// BitSet<10, u8> v;
    ...
    ///
    /// On return, `left` is a clone of the bits from the start of the bit-set up to but not including `at` and
    /// `right` contains the bits from `at` to the end of the bit-set. This bit-set itself is not modified.
    ///
    /// @note This method panics if the split point is beyond the end of the bit-set.
    ///
    /// # Example
