...
For example, if `v` is a `gf2::BitVec`, you can call `v.count_ones()` to count the number of set bits in the vector instead of calling the free function `gf2::count_ones(v)`, though both forms are valid.
...
The `gf2::BitVec` class provides a rich set of methods for constructing, querying, and manipulating, bit-vectors.
Here is an overview of the main methods available in the class:

| Category                                      | Description                                                                     |
| --------------------------------------------- | ------------------------------------------------------------------------------- |
| [Required Concept Methods](#vec-concept)      | Methods needed to satisfy the `gf2::BitStore` concept.                          |
...
| [Appending Elements](#vec-appending)          | Methods to append bits from various sources to the end of a bit-vector.         |
| [Removing Elements](#vec-removing)            | Methods to remove bits from the end of a bit-vector.                            |
| [Bit Access](#vec-bit-access)                 | Methods to access individual bit elements in a bit-vector.                      |
| [Vector Queries](#vec-queries)                | Methods to query the overall state of a bit-vector.                             |
| [Vector Mutators](#vec-mutators)              | Methods to mutate the overall state of a bit-vector.                            |
| [Vector Fillers](#vec-fillers)                | Methods to fill a bit-vector from various sources.                              |
| [Span Creators](#vec-span)                    | Methods to create non-owning views over a part of a bit-vector --- _bit-spans_. |
| [Sub-vector Extractors](#vec-sub-vectors)     | Methods to pull out a clone of piece of a bit-vector as new bit-vectors         |
...
| `gf2::BitVec::words`      | Returns the number of `Word`s needed to vector those elements.              |
...
| `gf2::BitVec::data const` | Returns a const pointer to the first `Word` holding bits in the vector.     |
| `gf2::BitVec::data`       | Returns a non-const pointer to the first `Word` holding bits in the vector. |
...
> [!WARNING]
> While the `gf2::BitVec::data` method provides write access to the underlying words, you must be careful when modifying them directly.
> You must ensure that any bits beyond the bit-vector's size remain zero.
> The `gf2::BitVec::set_word` method takes care of this for you, so prefer using that method when possible.
...
## Appending Elements {#vec-appending}
...
The `gf2::BitVec::append_digit` method appends bits from a character representing a digit in one of the bases 2, 4, 8, or 16.
It does nothing if it fails to parse the character.

## Removing Elements {#vec-removing}
...
The two `gf2::BitVec::split_off` methods complement the `gf2::BitVec::split_at` methods.
These versions change the size of the bit-vector _in place_.

## Bit Access {#vec-bit-access}

The following functions provide access to individual bit elements in the bit-vector.
...
The non-const version of `gf2::BitVec::operator[]()` returns a `gf2::BitRef`, which is "reference" to an individual bit in the vector.
...
## Vector Queries {#vec-queries}

The following functions let you query the overall state of a bit-vector.

| Function                      | Description                                              |
| ----------------------------- | -------------------------------------------------------- |
| `gf2::BitVec::is_empty`       | Returns true if the vector is empty                      |
| `gf2::BitVec::any`            | Returns true if _any_ bit in the vector is set.          |
| `gf2::BitVec::all`            | Returns true if _every_ bit in the vector is set.        |
| `gf2::BitVec::none`           | Returns true if _no_ bit in the vector is set.           |
...
## Vector Mutators {#vec-mutators}

The following functions let you mutate the entire vector in a single call.
...
## Vector Fillers {#vec-fillers}

The following functions let you populate the entire vector from multiple sources in a single call.

| Function                        | Description                                                       |
| ------------------------------- | ----------------------------------------------------------------- |
| `gf2::BitVec::copy_from`        | Copies the bits from various sources to this vector.              |
| `gf2::BitVec::fill_from`        | Fills the vector by repeatedly calling a function for each index. |
| `gf2::BitVec::fill_from_random` | Fills the vector with random 0's and 1's.                         |

The `gf2::BitVec::copy_from` functions support copying from:
...
-   _Any_ unsigned integer that has the same number of bits as the vector.
...
## Span Creators {#vec-span}

The following functions let you create a `gf2::BitSpan`, which is a non-owning view of some contiguous subset of bits in the vector.
...
## Sub-vector Extractors {#vec-sub-vectors}
...
| `gf2::BitVec::next_set`       | Returns the index of the next set bit in the vector _after_ the passed index.        |
| `gf2::BitVec::previous_set`   | Returns the index of the previous set bit in the vector _before_ the passed index.   |
...
| `gf2::BitVec::next_unset`     | Returns the index of the next unset bit in the vector _after_ the passed index.      |
| `gf2::BitVec::previous_unset` | Returns the index of the previous unset bit in the vector _before_ the passed index. |

## Iterators {#vec-iterators}

The following functions create iterators for traversing the bits or underlying words in the vector:
