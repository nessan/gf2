// Speed test on word-by-word convolution vs element-by-element version over a wide range of inputs.
// **Note:**  Needs to be run in release mode.
// SPDX-FileCopyrightText:  2025 Nessan Fitzmaurice <nzznfitz+gh@icloud.com>
// SPDX-License-Identifier: MIT
#include <print>
#include <chrono>
#include <random>
#include <utilities/stopwatch.h>
#include <gf2/namespace.h>
#include "naive.h"

int
main() {
    using vector_type = BitVec<u8>;

    // Convolving bit-vectors u and v with the following sizes.
    auto nu = 5000uz;
    auto nv = 5000uz;

    // Create the two input bit-vectors.
    auto u = vector_type::random(nu);
    auto v = vector_type::random(nv);

    // Space for the convolution results.
    vector_type w1, w2;

    // Number of trials & a tick size
    auto n_trials = 1000uz;
    auto n_tick = n_trials / 20;

    // And a timer
    utilities::stopwatch sw;

    // Do things the fast way
    std::print("Running {} calls of `convolve(u[{}], v[{}])`   ", n_trials, u.size(), v.size());
    sw.click();
    for (std::size_t n = 0; n < n_trials; ++n) {
        if (n % n_tick == 0) std::cout << '.' << std::flush;
        convolve(u, v);
    }
    sw.click();
    auto lap1 = sw.lap();
    std::print(" done.\n");

    // Do things the slower way
    std::print("Running {} calls of `naive::convolve(u[{}], v[{}])` ", n_trials, u.size(), v.size());
    sw.click();
    for (std::size_t n = 0; n < n_trials; ++n) {
        if (n % n_tick == 0) std::cout << '.' << std::flush;
        naive::convolve(u, v);
    }
    sw.click();
    auto lap2 = sw.lap();
    std::print(" done.\n");

    std::print("convolve loop time:      {:.2f}s.\n", lap1);
    std::print("naive::convolve loop time:    {:.2f}s.\n", lap2);
    std::print("ratio:                        {:.2f}.\n", lap2 / lap1);

    // Also check that the final result from the two methods matched.
    if (w1 != w2) {
        // They didn't so print a message and exit with an error-code ...
        std::print("METHOD MISMATCH, METHOD MISMATCH, METHOD MISMATCH, METHOD MISMATCH!\n");
        std::print("Input bit-vector u: {}\n", u);
        std::print("Input bit-vector v: {}\n", v);
        std::print("Naive convolution:  {}\n", w1);
        std::print("Faster convolution: {}\n", w2);
        exit(1);
    }
}
