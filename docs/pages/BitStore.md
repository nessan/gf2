...
`gf2::BitStore` is a [concept] that is satisfied by the library's three vector-like types:
...
The classes have _many_ methods in common.

The implementation of those methods forwards their work to the dozens of free functions we provide that operate on any type that satisfies the `gf2::BitStore` concept --- _bit-stores_.

The functions include bit accessors, mutators, fills, queries, iterators, stringification methods, bit-wise operators, arithmetic operators, and more. They are documented in detail later in this page and in the reference: `gf2::BitStore`.

> [!NOTE]
> For convenience, the free functions are pulled into the three vector-like classes as member functions.
...
| Required Method  | Return Type        | Expected Functionality                                    |
| ---------------- | ------------------ | --------------------------------------------------------- |
| `size() const`   | `usize`            | The number of accessible bits in the object.              |
| `data() const`   | `word_type const*` | A const pointer to the first real underlying word.        |
| `data()`         | `word_type*`       | A non-const pointer to the first real underlying word.    |
| `offset() const` | `u8`               | The offset in bits to the first element in `data()[0]`.   |
| `words() const`  | `usize`            | The _minimum_ number of words needed to store those bits. |
| `word(i) const`  | `word_type`        | A copy of "word" number `i`.                              |
| `set_word(i,v)`  | `void`             | Set the accessible bits in "word" `i` to those from `v`.  |

> [!NOTE]
> We could implement the last three methods above in terms of the first four.
> For example, the `words()` method could be implemented as a trivial computation based on `size()` and the number of bits per underlying word.
> However, all the concrete bit-stores already cache the required value, so we use that instead.
...
The methods are trivial to implement for `gf2::BitSet` and `gf2::BitVec`.

Here is a sketch of how they might work for the `gf2::BitVec` class:

```c++
template <Unsigned Word = usize> // <1>
class BitVec {
private:
    usize m_size; // Number of bits in the bit-vector
    std::vector<Word> m_data; // Underlying store of words holding the bits
public:
    using word_type = Word; // <2>

    constexpr usize size() const        { return m_size; } // <3>
    constexpr Word const* data() const  { return m_data.data(); }
    constexpr Word* data()              { return m_data.data(); }
    constexpr u8 offset() const         { return 0; }
    constexpr usize words() const       { return m_data.size(); }
    constexpr Word word(usize i) const  { return m_data[i]; }
    constexpr void set_word(usize i, Word v) { m_data[i] = v; }
};
```
...
3. The required `BitStore` methods are all trivially implemented, though the real implementation allows for range checks if the compile-time `DEBUG` flag is present. The `set_word` method also ensures that any unoccupied high-order bits in the final word remain zero.

A sketch of the `BitSet` class is similar, except that the underlying store is a `std::array` of words rather than a `std::vector`.
...
However, the many `BitStore` related functions operate _as if_ bit element 0 in the store is the **lowest-order** bit of "word" **0**.
...
They expect that `store.word(0)` holds the first 8 bits in the bit-store, `store.word(1)` has the following 8 bits, and `store.word(2)` holds the final four elements in its four lowest order bits.
It also expects that the four highest "unoccupied" bits in `store.word(2)` are set to 0.

If the store is a bitset or a bit-vector, the implementation of these `BitStore` expectations is easy.
...
`store.word[i]` will use some high-order bits from `w[j]` and low-order bits from `w[j+1]` as shown in the following example:
...
| [Store Queries](#store-queries)               | Functions to query the overall state of a bit-store.                             |
| [Store Mutators](#store-mutators)             | Functions to mutate the overall state of a bit-store.                            |
| [Store Fillers](#store-fillers)               | Functions to fill a bit-store from various sources.                              |
| [Span Creators](#store-spans)                 | Functions to create non-owning views over a part of a bit-store --- _bit-spans_. |
| [Sub-vector Extractors](#store-sub-vectors)   | Functions to pull out a clone of a piece of a bit-store as new bit-vectors       |
...
| [Stringification](#tore-stringification)      | Functions to create string representations of a bit-store.                       |
...
| Function           | Description                                                      |
| ------------------ | ---------------------------------------------------------------- |
| `gf2::copy`        | Copies the bits from various sources to this store.              |
| `gf2::fill`        | Fills the store by repeatedly calling a function for each index. |
| `gf2::fill_random` | Fills the store with random 0's and 1's.                         |

The `copy` functions support copying from:
...
## Sub-vector Extractors {#store-sub-vectors}
...
| Function           | Description                                                                        |
| ------------------ | ---------------------------------------------------------------------------------- |
| `gf2::bits`        | Returns a `gf2::BitsIter` iterator over the bits in the store.                     |
| `gf2::set_bits`    | Returns a `gf2::SetBitsIter` iterator to view the indices of all the set bits.     |
| `gf2::unset_bits`  | Returns a `gf2::UnsetBitsIter` iterator to view the indices of all the unset bits. |
| `gf2::store_words` | Returns a `gf2::WordsIter` iterator to view the "words" underlying the store.      |
...
1. Returns a mutable `gf2::BitsIter` that allows modification of the bits in the store.
2. Returns an immutable `gf2::BitsIter` that only allows one to view the bits in the store.

## Stringification {#tore-stringification}
...
-   [`BitsIter`](BitsIter.md) for iterators over the bits in a bit-store.
-   [`SetBitsIter`](SetBitsIter.md) for iterators over the indices of set bits in a bit-store.
-   [`UnsetBitsIter`](UnsetBitsIter.md) for iterators over the indices of unset bits in a bit-store.
-   [`WordsIter`](WordsIter.md) for iterators over the underlying "words" in a bit-store.
