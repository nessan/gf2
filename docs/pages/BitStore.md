...
| `offset() const` | `u8`               | The offset in bits to the first element in `store()[0]`.  |
...
private:
    ...
    std::vector<Word> m_store; // Underlying store of words holding the bits
public:
    ...
    constexpr void set_word(usize i, Word v) { m_store[i] = v; }
};
```
...
| [Sub-vectors](#store-sub-vectors)             | Functions to pull out a clone of a piece of a bit-store as new bit-vectors       |
...
| `gf2::random_fill` | Fills the store with random 0's and 1's.              |
