...
| [Constructors](#set-constructors)             | Methods to create bit-sets with various initial values..                     |
| [Factory Constructors](#set-factory)          | Class methods to create bit-sets with specific fills.                        |
| [Bit Access](#set-bit-access)                 | Methods to access individual bit elements in a bit-set.                      |
| [Queries](#set-queries)                       | Methods to query the overall state of a bit-set.                             |
| [Mutators](#set-mutators)                     | Methods to mutate the overall state of a bit-set.                            |
| [Fills](#set-fills)                           | Methods to fill a bit-set from various sources.                              |
| [Spans](#set-span)                            | Methods to create non-owning views over a part of a bit-set --- _bit-spans_. |
| [Sub-vectors](#set-sub-vectors)               | Methods to pull out a clone of piece of a bit-set as new bit-vector.         |
...
> [!WARNING]
> While the `gf2::BitSet::store` method provides write access to the underlying words, you must be careful when modifying them directly.
...
The default constructor for a `gf2::BitSet<N>` creates an a bit-set with `N` bits, all initialised to zero.
