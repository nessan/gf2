...
| --------------------------------------------- | --------------------------------------------------------------------------------- |
| [Required Concept Methods](#span-concept)     | Methods needed to satisfy the `gf2::BitStore` concept.                            |
...
| [Span Queries](#span-queries)                 | Methods to query the overall state of a bit-span.                                 |
| [Span Mutators](#span-mutators)               | Methods to mutate the overall state of a bit-span.                                |
| [Span Fillers](#span-fillers)                 | Methods to fill a bit-span from various sources.                                  |
| [Span Creators](#span-span)                   | Methods to create non-owning views over a part of a bit-span --- _sib-bit-spans_. |
| [Sub-vector Extractors](#span-sub-vectors)    | Methods to pull out a clone of piece of a bit-span as new bit-vector.             |
...
These methods were trivial to implement for bit sets and vectors but require some thought for bit spans.
...
Here `BitXXX` can be any class that satisfies the `gf2::BitStore` concept, such as `gf2::BitVec`, `gf2::BitSet`, or even `gf2::BitSpan` as taking a span of a span is supported.
...
The following functions provide access to individual bit elements in the bit-span.
...
The following functions let you query the overall state of a bit-span.
...
## Span Mutators {#span-mutators}

The following functions let you mutate the entire bit-span in a single call.

| Function                      | Description                                       |
| ----------------------------- | ------------------------------------------------- |
| `gf2::BitVecSetVec::flip_all` | Flips the values of all the bits in the bit-span. |

They efficiently operate on words at a time, so they are inherently parallel.

## Span Fillers {#span-fillers}

The following functions let you populate the entire bit-span from multiple sources in a single call.

| Function                         | Description                                                         |
| -------------------------------- | ------------------------------------------------------------------- |
| `gf2::BitSpan::copy_from`        | Copies the bits from various sources to this bit-span.              |
| `gf2::BitSpan::fill_from`        | Fills the bit-span by repeatedly calling a function for each index. |
| `gf2::BitSpan::fill_from_random` | Fills the bit-span with random 0's and 1's.                         |

The `gf2::BitSpan::copy_from` functions support copying from:
...
-   _Any_ unsigned integer that has the same number of bits as the bit-span.
...
> You can always use a `gf2::BitSpan` sub-span to copy a subset of bits if needed.
...
## Span Creators {#span-span}

The following functions let you create a `gf2::BitSpan` from a bit-span --- a _sub-bit-span_.
...
## Sub-vector Extractors {#span-sub-vectors}
...
The following functions create iterators for traversing the bits or underlying words in the bit-span:
...
| Function                    | Description                                                                        |
| --------------------------- | ---------------------------------------------------------------------------------- |
| `gf2::BitSpan::bits`        | Returns a `gf2::BitsIter` iterator over the bits in the bit-span.                  |
| `gf2::BitSpan::set_bits`    | Returns a `gf2::SetBitsIter` iterator to view the indices of all the set bits.     |
| `gf2::BitSpan::unset_bits`  | Returns a `gf2::UnsetBitsIter` iterator to view the indices of all the unset bits. |
| `gf2::BitSpan::store_words` | Returns a `gf2::WordsIter` iterator to view the "words" underlying the bit-span.   |
...
1. Returns a mutable `gf2::BitsIter` that allows modification of the bits in the bit-span.
2. Returns an immutable `gf2::BitsIter` that only allows one to view the bits in the bit-span.
