#!/usr/bin/env python3
#
# The environment should have the Python packages `numpy` & `sympy` installed
#
...
file_name = input(f"Output file to create [default '{default_file}']: ") or default_file

try:
    f = open(file_name, "w")
except OSError as e:
    print(
        f"Failed to open '{file_name}' for writing (system error = {e.errno})!",
        file=sys.stderr,
    )
    sys.exit(e.errno)
...
default_min = 10
n_min = input(f"Smallest matrix to generate [default {default_min}]: ") or default_min
try:
    n_min = abs(int(n_min))
except ValueError:
    print(
        f"Failed to parse '{n_min}' as int so will use default of {default_min}!",
        file=sys.stderr,
    )
    n_min = default_min
...
default_max = 50
n_max = input(f"Largest matrix to generate [default {default_max}]: ") or default_max
try:
    n_max = abs(int(n_max))
except ValueError:
    print(
        f"Failed to parse '{n_max}' as int so will use default of {default_max}!",
        file=sys.stderr,
    )
    n_max = default_max
...
default_step = 10
n_step = input(f"Step size for matrix sizes [default {default_step}]: ") or default_step
try:
    n_step = abs(int(n_step))
except ValueError:
    print(
        f"Failed to parse '{n_step}' as int so will use default of {default_step}!",
        file=sys.stderr,
    )
    n_step = default_step
...
default_prob = 0.5
prob = (
    input(f"Probability of getting set bits in the matrices [default {default_prob}]: ")
    or default_prob
)
try:
    prob = float(prob)
    ...
    print(
        f"Failed to parse '{prob}' as a float so will use default of {default_prob}!",
        file=sys.stderr,
    )
    prob = default_prob
...
with f:
    ...
    print(
        f"# File contains {n_matrices} binary matrices and their GF2 characteristic polynomials",
        file=f,
    )
    print(
        f"# In each case, the matrix elements are set to one with probability {prob}",
        file=f,
    )
    print(
        "# The data was generated in Python using the charpoly() method from the Sympy package",
        file=f,
    )
    print(file=f)
    ...
    for n in range(n_min, n_max + 1, n_step):
        print(f"Processing a {n:3} x {n:3} matrix ... ", end="", flush=True)
        ...
        print(f"completed in {comp_time:.3f}s", flush=True)
        ...
        print(
            f"# Sympy characteristic polynomial coefficients computed in {comp_time:.3f} seconds:",
            file=f,
        )
        print(*c, sep="", file=f)
        print(file=f)
