...
The `<gf2/assert.h>` header includes macros for checking a boolean condition or comparing two values.

On failures, the macros print a customisable message to the standard error stream and then, by default, exit the program with a non-zero status code.

> [!TIP]
> You can set the `gf2::exit_on_assert_failure` variable to `false` to prevent the program from exiting on assertion failures.
> That can be useful in some testing scenarios.

Failed assertions show the source code location where `gf2_assert` was called, along with a stringified version of the condition that caused the failure.
...
```cpp
gf2_always_assert(condition, ...)   // <1>
gf2_debug_assert(condition, ...)    // <2>
gf2_assert(condition, ...)          // <3>
```
...
2. The `gf2_debug_assert` macro expands to `gf2_always_assert` **if** the `DEBUG` flag is set; otherwise, it's a no-op.
3. The `gf2_assert` macro expands to `gf2_always_assert` **unless** the `NDEBUG` flag is set, which is a no-op. <br>This is similar to the [standard `assert`] macro.
...
```cpp
gf2_always_assert_eq(lhs, rhs, ...)     // <1>
gf2_debug_assert_eq(lhs, rhs, ...)      // <2>
assert_eq(lhs, rhs, ...)             // <3>
```
...
2. The `gf2_debug_assert_eq` macro expands to `gf2_always_assert_eq` **if** the `DEBUG` flag is set; otherwise, it's a no-op.
3. The `assert_eq` macro expands to `gf2_always_assert_eq` **unless** the `NDEBUG` flag is set, which is a no-op similar to the [standard `assert`] macro.
...
For the sake of the example, we will use the `gf2_always_assert_eq` macro to check for equality.
More typically, you use either the `gf2_debug_assert_eq` or the `assert_eq` macro.

```cpp
#include <gf2/assert.h>
int subtract(int x, int y)
{
    gf2_always_assert_eq(x, y, "well that didn't work but x - y = {}", x - y);
    return y - x;
}
int main()
{
...
```text
FAILED: `gf2_assert_eq(x, y)` [file.cpp:line-number]
...
The `gf2_always_assert` and `gf2_always_assert_eq` macros are used to verify that a condition is true and to exit with an informative message if it is not. <br>
...
The `gf2_debug_assert` and `gf2_debug_assert_eq` macros cover this type of verification.
Enabling these checks requires the programmer to take a specific action: she must set the `DEBUG` flag at compile time.

For example, here is a typical precondition from `dot(const gf2::BitStore &u, const gf2::BitStore &v)` function:

```cpp
gf2_debug_assert_eq(u.size(), v.size(), "Incompatible sizes!");
```

This code checks that the two arguments have equal length --- a necessary constraint for the dot product operation to make sense.
...
The decision to use one form instead of the other is predicated on the cost of doing the check versus the work done by the method in question.

A primary use case for `gf2_debug_assert` and `gf2_debug_assert_eq` is checking index bounds.
...
Also, macros can completely "disappear" by setting appropriate compiler flags.

WARNING: Macros depend on comma placement to separate arguments, which can be a gotcha in some cases.
