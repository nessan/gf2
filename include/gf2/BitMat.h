...
public:
    ...
    static constexpr BitMat companion(Store const& top_row) {
        ...
        result.m_rows[0].copy_from(top_row);
    ...
    static std::optional<BitMat> from_row_store(Store const& v, usize r) {
        ...
        for (auto i = 0uz; i < r; ++i) {
            auto begin = i * c;
            auto end = begin + c;
            result.m_rows[i].copy_from(v.span(begin, end));
        }
        return result;
    }
    ...
    constexpr BitMat sub(usize r_start, usize r_end, usize c_start, usize c_end) const {
        ...
        BitMat result{r, c};
        for (auto i = 0uz; i < r; ++i) result.m_rows[i].copy_from(m_rows[i + r_start].span(c_start, c_end));
    ...
    constexpr void replace(usize top, usize left, BitMat<Word> const& src) {
        ...
        for (auto i = 0uz; i < r; ++i) m_rows[top + i].span(left, left + c).copy_from(src.m_rows[i]);
    }
...
///
/// @note We store bit-matrices by rows so `dot(BitMat, BitVec)` will generally be faster than this.
