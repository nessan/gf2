...
static_assert(std::bidirectional_iterator<SetBitsIter<std::size_t>>);

// Check that we covered all the necessary bases for the `UnsetBits` iterators to be bidirectional.
static_assert(std::bidirectional_iterator<UnsetBitsIter<std::size_t>>);

// Check that we covered all the necessary bases for the const and non-const `BitsIter` iterators to be bidirectional.
...
static_assert(std::bidirectional_iterator<BitsIter<store_type, false>>);
static_assert(std::bidirectional_iterator<BitsIter<store_type, true>>);
