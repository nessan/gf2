...
The `gf2` library operates by packing individual bit elements into the primitive unsigned words that are natively addressed by computer.
We have a number of utility functions that operate on those words.

## Concept

The `gf2::unsigned_word` concept is satisfied by any of the computer's primitive unsigned types.
